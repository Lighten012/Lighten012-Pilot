...
import (
	"encoding/json"
	"flag"
	"github.com/Lighten012/Lighten012-Pilot/internal/control"
...
func newHandler(webDir string) http.Handler {
	return handler(webDir, "/run/pilot-netd/control.sock")
}
func handler(webDir, socket string) http.Handler {
	mux := http.NewServeMux()
	controller := control.New(socket)
	...
	mux.HandleFunc("GET /api/health", func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json; charset=utf-8")
		w.Header().Set("Cache-Control", "no-store")
		json.NewEncoder(w).Encode(map[string]string{"status": "ok", "version": "0.6.0", "mode": "prototype", "hostname": hostname, "architecture": runtime.GOARCH, "os": runtime.GOOS})
	})
...
func main() {
	...
	flag.Parse()
	socket := os.Getenv("PILOT_NET_SOCKET")
	if socket == "" {
		socket = "/run/pilot-netd/control.sock"
	}
	server := &http.Server{Addr: *listen, Handler: handler(*web, socket), ReadHeaderTimeout: 5 * time.Second, ReadTimeout: 10 * time.Second, WriteTimeout: 120 * time.Second, IdleTimeout: 60 * time.Second}
